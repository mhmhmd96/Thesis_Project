...
def get_dense_model():
    ...
    conv.trainable = True
    num_trainable=100
    for i in conv.layers[-num_trainable:]:
        i.trainable = True
    #for i in conv.layers[:-num_trainable]:
    #    i.trainable = False
    model = tf.keras.models.Sequential([
...
class CifarClient(fl.client.NumPyClient):
    ...
    def get_properties(self, config):
    	...
    	print("Free RAM: ", free_ram / 1e+9, "GB")
    	
    	# Used CPU percentage
    	used_cpu = psutil.cpu_percent(cpu_duration)
    	cpu_count = psutil.cpu_count()
    	print("CPU Usage: ", used_cpu, "%")
    	
    	properties = {'freeMEM':free_ram,'usedCPU':used_cpu, 'numCPU':cpu_count,'delay':self.delay}
    	return properties
    ...
    def get_delay(self):
        ...
        if not q.empty():
            ...
            if self.i%2==0:
                delay = (delay+3*self.previous_delay)/4
                self.delay = delay
                event.set()
                time.sleep(0.5)
                event.clear()
    ...
    def get_eval_index(self, ram, cpu, cpu_count, delay):
        if delay > max_delay:
            delay=max_delay
        ...
        # Weights
        computational_weight = {'RAM': 0.5, 'CPU': 0.5}
    ...
    def ping_host1(self):
        ...
        while i<=200 and not event.is_set():
            ping_result = ping(target=SERVER_IP, count=ping_count, timeout=5)
...
def main() -> None:
    ...
    fl.client.start_numpy_client(server_address=SERVER_IP+":"+SERVER_PORT, client=client, )
    event.set()
